...
def main(ini_path, log_level=logging.WARNING, mnid_to_run='ALL',
    ...
    if cfg.avg_monthly_met_flag:
        avg_monthly_header = 'Met Node ID,Met Node Name,Jan,Feb,Mar,Apr,May,' \
                             'Jun,Jul,Aug,Sep,Oct,Nov,Dec'
        avgTMaxRev_path = cfg.input_met['avgm_tmax_path'].replace(
                ".", "_rev.")
    ...
    if mp_procs > 1:
        if not cfg.avg_monthly_met_flag:
            if cfg.output_met_flag and \
                    cfg.output_met['data_structure_type'].upper() != 'SF P':
                logging.warning(
                    "Met output data structure type " + cfg.output_met[
                        'data_structure_type'] + " can not yet be created "
    ...
    # loop through met nodes meta data
    logging.warning("\n")
    met_node_count = 0
    for met_node_id, met_node in sorted(mnd.met_nodes_data.items()):
        if mnid_to_run == 'ALL' or mnid_to_run == met_node_id:
            logging.info('  Processing node id' + met_node_id + ' with name ' +
                         met_node.met_node_name)
        if met_node.TR_b0 is None:
                met_node.TR_b0 = cfg.input_met['TR_b0']
        if met_node.TR_b1 is None:
                met_node.TR_b1 = cfg.input_met['TR_b1']
        if met_node.TR_b2 is None:
                met_node.TR_b2 = cfg.input_met['TR_b2']

    # read input met data
        met_node_count += 1
        if node_mp_flag and cfg.output_met[
            'data_structure_type'].upper() == 'SF P' and met_node_count > 1:
            node_mp_list.append([met_node_count, cfg, met_node, mnd])
        else:
            if not met_node.read_and_fill_met_data(met_node_count, cfg,
                                                   mnd):
                if cfg.avg_monthly_met_flag:
            ...
            if cfg.avg_monthly_met_flag:
                avgTMaxRev_string = met_node_id + cfg.input_met[
                    'avgm_tmax_delimitor'] + met_node.met_node_name
                avgTMinRev_string = avgTMaxRev_string
                for month, row in met_node.input_met_df.groupby(
                        ['month'])['tmax'].agg([np.mean]).iterrows():
                    avgTMaxRev_string = avgTMaxRev_string + cfg.input_met[
                        'avgm_tmax_delimitor'] + str(row['mean'])
                for month, row in met_node.input_met_df.groupby(
                        ['month'])['tmin'].agg([np.mean]).iterrows():
                    avgTMinRev_string = avgTMinRev_string + cfg.input_met[
                'avgm_tmax_delimitor'] + str(row['mean'])
            ...
            if cfg.output_met_flag:
                if not met_node.setup_output_met_data(met_node_count, cfg,
                                                      mnd):
                    if cfg.avg_monthly_met_flag:
