...
class refET:
    ...
    def ComputeRs(self, doy, tmax, tmin, TDew, elevm, latitude, avgm_tmax, avgm_tmin):
        ...
        Args:
            ...
            latitude: latitude
            avgm_tmax: average monthly maximum temperature
            avgm_tmin: average monthly minimum temperature
        ...
        Rs = self.EstimateIncidentRadiation(Rso, tmax, tmin, avgm_tmax, avgm_tmin)
        return Rs
    ...
    def ComputeHargreavesSamaniRefET(self, doy, tmax, tmin, latitude):
        ...
        latentHeat = self.aFNHarrison(tmean)
    ...
    def ComputePenmanRefETs(self, yr, mo, da, doy, time_step, tmax, tmin, TDew, Rs, u24, elevm, latitude):
        ...
        penmans = self.Penmans(latitude, doy, pressure, Rn56, Rn82, ed, ea, latentHeat, Delta, Gamma, tmean, G82, G82o, GFAO, GFAOr, u242)
        # Penman, PreTay, KimbPeng, ASCEPMstdr, ASCEPMstdo, FAO56PM, KimbPen = penmans
        return penmans

    def aFNHarrison(self, tmean):
        ...
        """
        #aFNHarrison = 2500000.0! - 2360 * tmean  # '!' forces single precision, numpy Float32 is option if needed
        a = 2500000.0 - 2360 * tmean  # J/Kg   Latent Heat of Vaporization
        return a
    ...
    def aFNDelta(self, tmean):
        """ Eq. 4 + Eq. 7"""
        a = 4098 * self.aFNEs(tmean) / (tmean + 237.3) / (tmean + 237.3) # kPa/C
        return a
...
def do_tests():
    ...
    print 'TESTS\n'
    tmean = 51.0
    latentHeat = o.aFNHarrison(tmean)
    print 'aFNHarrison(T=%s):                          %s' % (tmean, latentHeat)
    ...
    tmax, tmin = 62.0, 34.3
    avgm_tmax, avgm_tmin = 60.0, 30.3
    print
    print 'EstimateIncidentRadiation(Rso, tmax, tmin, avgm_tmax, avgm_tmin)'
    Rs = o.EstimateIncidentRadiation(Rso, tmax, tmin, avgm_tmax, avgm_tmin)
    print 'EstimateIncidentRadiation(%s,%s,%s,%s,%s):        %s' % (Rso, tmax, tmin, avgm_tmax, avgm_tmin, Rs)
    ...
    # time_step = 'hour'
    time_step = 'day'
    G82, G82o, GFAO, GFAOr = o.ComputeSoilHeat(da, time_step, tmean, Rn56)
    print 'ComputeSoilHeat(%s,%s,%s,%s,%s):  %s, %s, %s, %s' % (da, time_step, tmean, Rn56, G82, G82o, GFAO, GFAOr)

    ea = (o.aFNEs(tmax) + o.aFNEs(tmin)) * 0.5
    u242 = 5   # wind ??
    penman_results = o.Penmans(latitude, doy, pressure, Rn56, Rn82, ed, ea, latentHeat, Delta, Gamma, tmean, G82, G82o, GFAO, GFAOr, u242)
