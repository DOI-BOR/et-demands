...
RefET
-----
The RefET module calculates reference ET from daily weather data using the approach described in ASCE-NWRI `(2005) <https://doi.org/10.1061/9780784408056>`_. If needed, it estimates solar radiation, dew point temperature from mean monthy dew point depression, and gap fills maximum and minimum daily air temperature and mean daily wind speed using mean monthly values.

CAN BE RUN HOURLY OR DAILY - WHAT VARS ARE NEEDED IF RUN HOURLY?
...
\ :sup:`a` If T\ :sub:`d` is not provided, mean monthly dew point depression, K\ :sub:`0`, must be provided.

\ :sup:`b` Wind measurement height in meters must be provided in the *ini* file.

\ :sup:`c` If R\ :sub:`n` is not provided it will be estimated using the approach described in Thornton and Running `(1998) <https://doi.org/10.1016/S0168-1923(98)00126-9>`_. The three Thornton and Running coefficients must be provided in the *ini* file. (LINK TO TR section in model description) (also LINK to TR calibration section)
...
|                 |                |                                        |                                                                          |
|                 |                | m d\ :sup:`-1`                         | m/d, m/day, mpd                                                               |
|                 |                |                                        |                                                                          |
|                 |                | mi d\ :sup:`-1`                        | miles/d, miles/day                                                       |
|                 |                |                                        |                                                                          |
+-----------------+----------------+----------------------------------------+--------------------------------------------------------------------------+
...
*T\ :sub:`max`* example shown. File structure will be the same for T\ :sub:`min`, u\ :sub:`x`, and K\ :sub:`0`.
...
+-----------------+----------------+----------------------------------------+--------------------------------------------------------------------------+
| Elevation       | z              | meters                                 | m, meter                                                                 |
|                 |                |                                        |                                                                          |
|                 |                | feet                                   | feet                                                                     |
+-----------------+----------------+----------------------------------------+--------------------------------------------------------------------------+

.. _model-outputs-cropet:
